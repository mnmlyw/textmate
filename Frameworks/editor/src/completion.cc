...
namespace ng
{
	...
	std::vector<std::string> editor_t::completions (size_t bow, size_t eow, std::string const& prefix, std::string const& suffix, std::string const& scopeAttributes)
	{
		...
		if(std::string const* str = std::get_if<std::string>(&value))
		{
		...
		if(plist::array_t const* completions = std::get_if<plist::array_t>(&completionsValue))
		{
			for(size_t i = 0; i < completions->size(); ++i)
			{
				if(std::string const* word = std::get_if<std::string>(&(*completions)[i]))
