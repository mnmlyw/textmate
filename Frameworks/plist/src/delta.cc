...
namespace
{
	struct test_equal_helper_t
	{
		...
		{
			if(T const* value = std::get_if<T>(&rhs))
				return lhs == *value;
			return false;
		}
	};
...
static void delta_plist_helper (plist::dictionary_t const& oldDict, plist::dictionary_t const& newDict, plist::dictionary_t& changed, plist::array_t& deleted, std::vector<std::string> path = std::vector<std::string>())
{
	...
	for(auto const& newValue : newDict)
	{
		...
		if(std::get_if<plist::dictionary_t>(&newValue.second) && oldValue != oldDict.end() && std::get_if<plist::dictionary_t>(&oldValue->second))
		{
			delta_plist_helper(std::get<plist::dictionary_t>(oldValue->second), std::get<plist::dictionary_t>(newValue.second), changed, deleted, newPath);
		}
...
static void erase_key_path (plist::dictionary_t& plist, std::string const& keyPath)
{
	...
	for(auto key = v.begin(); key != v.end() && current; )
	{
		...
		if(++key == v.end())
			current->erase(it);
		else if(current = std::get_if<plist::dictionary_t>(&it->second))
			continue;
		else if(plist::array_t* array = std::get_if<plist::array_t>(&it->second))
		{
			for(auto it = array->begin(); it != array->end(); ++it)
			{
				if(std::string* str = std::get_if<std::string>(&(*it)))
				{
					if(*str == *key)
					{
						array->erase(it);
...
static void update_key_path (plist::dictionary_t& plist, std::string const& keyPath, plist::any_t const& value)
{
	...
	for(auto key = v.begin(); key != v.end(); )
	{
		...
		{
			if(current = std::get_if<plist::dictionary_t>(&it->second))
				continue;
			else if(plist::array_t* array = std::get_if<plist::array_t>(&it->second))
				array->push_back(*key);
			else
	...
	dictionary_t merge_delta (std::vector<dictionary_t> const& plists)
	{
		...
		riterate(it, plists)
		{
			...
			if(plist.find("isDelta") != plist.end())
			{
				...
				if(array_t const* deleted = deletedIt != plist.end() ? std::get_if<array_t>(&deletedIt->second) : nullptr)
				{
					for(auto item : *deleted)
					{
						if(std::string const* str = std::get_if<std::string>(&item))
							erase_key_path(res, *str);
					}
				}

				auto changedIt = plist.find("changed");
				if(dictionary_t const* changed = changedIt != plist.end() ? std::get_if<dictionary_t>(&changedIt->second) : nullptr)
				{
