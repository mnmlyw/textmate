...
namespace plist
{
	...
	{
		std::vector<any_t>& ref = std::get< std::vector<any_t> >(res = std::vector<any_t>());
	...
	{
		std::map<std::string, any_t>& ref = std::get< std::map<std::string, any_t> >(res = std::map<std::string, any_t>());
	...
	dictionary_t convert (CFPropertyListRef plist)
	{
		if(plist && CFGetTypeID(plist) == CFDictionaryGetTypeID())
		{
			...
			return std::get<dictionary_t>(res);
		}
		return dictionary_t();
	}
	...
	namespace
	{
		struct create_cf_property_list_t
		{
			...
			CFPropertyListRef operator() (plist::array_t const& array) const
			{
				...
				{
					CFPropertyListRef value = std::visit(*this, it);
			...
			CFPropertyListRef operator() (plist::dictionary_t const& dict) const
			{
				...
				for(auto const& it : dict)
				{
					...
					CFPropertyListRef value = std::visit(*this, it.second);
	...
	{
		return std::visit(create_cf_property_list_t(), plist);
	}
	...
	namespace
	{
		struct equal_helper_t
		{
			...
			bool operator() (plist::array_t const& lhs, plist::array_t const& rhs) const
			{
				...
				{
					if(!std::visit(*this, *lhsIter, *rhsIter))
						break;
				}
			...
			bool operator() (plist::dictionary_t const& lhs, plist::dictionary_t const& rhs) const
			{
				...
				for(; lhsIter != lhs.end() && rhsIter != rhs.end(); ++lhsIter, ++rhsIter)
				{
					if(lhsIter->first != rhsIter->first || !std::visit(*this, lhsIter->second, rhsIter->second))
						break;
				}
				return lhsIter == lhs.end() && rhsIter == rhs.end();
			}
	...
	{
		return std::visit(equal_helper_t(), lhs, rhs);
	}
	...
	{
		dictionary_t const* dict = std::get_if<dictionary_t>(&plist);
		if(!dict)
			return false;
		...
		if(sep == std::string::npos)
				return std::visit(convert_to_helper_t<T>(ref), it->second);
	...
	{
		bool flag;
		return std::visit(convert_to_helper_t<bool>(flag), item) && flag;
	}
