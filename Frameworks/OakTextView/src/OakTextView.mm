...
{
	if(plist::dictionary_t const* dict = std::get_if<plist::dictionary_t>(&action))
	{
...
+ (void)initialize
{
	...
	dispatch_once(&onceToken, ^{
		...
		for(auto const& info : KeyBindingLocations)
		{
			...
			{
				if(info.local || std::get_if<plist::dictionary_t>(&pair.second))
		...
		{
			if(std::string const* selector = std::get_if<std::string>(&pair.second))
...
{
	AUTO_REFRESH;
	if(std::string const* selector = std::get_if<std::string>(&anAction))
	{
		[self doCommandBySelector:NSSelectorFromString([NSString stringWithCxxString:*selector])];
	}
	else if(plist::array_t const* actions = std::get_if<plist::array_t>(&anAction))
	{
		std::vector<std::string> selectors;
		for(auto const& it : *actions)
		{
			if(std::string const* selector = std::get_if<std::string>(&it))
	...
	else if(plist::dictionary_t const* nested = std::get_if<plist::dictionary_t>(&anAction))
	{
...
- (ng::indent_correction_t)indentCorrections
{
	...
	if(std::string const* str = std::get_if<std::string>(&indentCorrections))
	{
		if(*str == "emptyLines")
