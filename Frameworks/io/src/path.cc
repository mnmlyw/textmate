...
namespace path
{
	...
	passwd* passwd_entry ()
	{
		...
		while(!entry || !entry->pw_dir || access(entry->pw_dir, R_OK) != 0) // Home folder might be missing <rdar://10261043>
		{
			...
			{
				io::spawn(std::vector<std::string>{ "/usr/bin/open", "https://openradar.appspot.com/10261043" });
			}
		}
		return entry;
	}
	...
	static std::string system_directory (int name, std::string const& file, std::string const& content)
	{
		...
		if(file != NULL_STR)
		{
			...
			{
				// Only reserve a unique name; unlike mktemp(3) this avoids the
				// race between checking and creating the path, but callers of
				// this branch (no content) expect the path to not yet exist.
				if(int fd = mkstemp(&str[0]); fd != -1)
				{
					close(fd);
					unlink(str.c_str());
				}
