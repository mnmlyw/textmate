...
static proxy_settings_t user_pw_settings (CFStringRef server, CFNumberRef portNumber)
{
	...
	if(err == errSecSuccess)
	{
		...
		for(CFIndex i = 0; user == NULL_STR && i < numResults; ++i)
		{
			...
			CFDataRef   passwordData = (CFDataRef)CFDictionaryGetValue(item, kSecValueData);
			if(account && passwordData)
			{
				user = cf::to_s(account);
				pw   = std::string((char const*)CFDataGetBytePtr(passwordData), CFDataGetLength(passwordData));
			}
		}
