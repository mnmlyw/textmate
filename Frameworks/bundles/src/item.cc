...
namespace bundles
{
	...
	bool item_t::initialize (plist::dictionary_t const& plist)
	{
		...
		for(auto const& pair : plist)
		{
			...
			{
				if(std::string const* str = std::get_if<std::string>(&pair.second))
					_scope_selector = *str;
			}
			...
			{
				if(plist::array_t const* array = std::get_if<plist::array_t>(&pair.second))
				{
					for(auto const& any : *array)
					{
						if(std::string const* str = std::get_if<std::string>(&any))
							_fields.emplace(pair.first, *str);
					}
				}
			}
			else if(pair.first == kFieldSettingName)
			{
				...
				else if(plist::array_t const* array = std::get_if<plist::array_t>(&pair.second))
				{
			...
			else if(pair.first == kFieldRequiredItems && std::get_if<plist::array_t>(&pair.second))
			{
				for(auto const& dict : std::get<plist::array_t>(pair.second))
				{
					std::string name;
					oak::uuid_t uuid;
	...
	plist::dictionary_t erase_false_values (plist::dictionary_t const& plist)
	{
		...
		for(auto const& pair : plist)
		{
			if(!std::get_if<bool>(&pair.second) || std::get<bool>(pair.second))
